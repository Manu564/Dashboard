...
import { CardSumaryProps } from "./CarfSumary.types";
import { cn } from "@/lib/utils";
import { Average } from "next/font/google";

export function CardSumary(props: CardSumaryProps) {
  const { icon: Icon, total, average, title, totaltipText } = props;
  return (
    <div className="shadow-sm bg-background rounded-lg p-5 py-3 hover:shadow-lg transition">
      ...
      <div className="flex gap-4 mt-2 md:mt-4">
        ...
        <div
          className={cn(
            "flex items-center gap-1 px-2 text-xs text-white rounded-lg h-[20px] bg-black dark:bg-secendary"
          )}
        >
          {average}%
          {average < 20 && (
